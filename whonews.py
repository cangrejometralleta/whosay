...
--provider switches to Anthropic or OpenAI, reading ANTHROPIC_API_KEY /
...
def format_model_error(provider, e):
    ...
    if provider == "ollama":
        return ("whonews: no answer from the local model at {} ({}). "
                 "Is `llama-server` running?".format(resolve_ollama_host(), e))
...
def resolve_ollama_host():
    """ResolveOllamaHost Returns the local llama-server Url, from $LLAMA_HOST or the local Default."""
    host = os.environ.get("LLAMA_HOST", "127.0.0.1:8080")
...
def _call_ollama(persona, prompt, model, timeout):
    ...
    url = resolve_ollama_host() + "/v1/chat/completions"
