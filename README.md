...
`whonews.py` and `whoopinion.py` are separate, stdlib-only scripts — they
aren't bundled into the `whosay` binary or symlink above. Link them the
same way:

```bash
ln -sf "$PWD/whonews.py" ~/.local/bin/whonews
ln -sf "$PWD/whoopinion.py" ~/.local/bin/whoopinion
whonews
whoopinion "it works"
```
...
./build.sh                  # all three binaries, or one at a time:
...
It lands in each generated `.spec` as `pathex=['.']` — those are build
artifacts, rewritten on every run and gitignored, so edit `build.sh`, not
them. `whoopinion` pulls in `whonews.py` the same way, for the backends.
...
`build.sh` builds all three. On its own, that first command only builds
`whosay` — the other two are separate binaries, compiled the same way:
...
```bash
python3 whoopinion.py "are tabs better than spaces?"
```
...
writes to a pipe becomes a question:
...
When both are given the argument wins and the pipe is ignored, the same rule
`whosay` follows. An empty question exits 1 rather than asking the model
nothing.
...
It takes every model flag `whonews` does — `--provider`, `--anthropic-key`,
`--anthropic-url`, `--openai-key`, `--openai-url`, `--ollama-url`, `--model`,
`--timeout` — from the same `add_model_arguments()` builder, so the two can't
drift apart, plus the `whosay` look flags `-s`/`-m`/`-b`, `-c`/`-a`/`-n`,
`-W`. The same environment defaults apply (`WHONEWS_PROVIDER`,
`WHONEWS_MODEL`, the credentials and the endpoints).

There is no cache and no archive: a question is asked once and answered once.
When the model stays silent — server down, key wrong, network out — the
character falls back to its `fallback` line, then to its `signature` phrase,
and the error says `whoopinion:` so you know which script spoke.

The answers are generated by a language model as a parody. They are not
opinions from, or endorsed by, any real person.

## Shared code: `whocast.py`

`whosay.py`, `whonews.py` and `whoopinion.py` are three CLIs over one
module. `whocast.py` holds what all of them need, and none of them is the
others' library:
...
- **the bubble** — `render_speech_bubble()`, `render_bubble_tail()` and
  `print_character_panel()`, the panel all three scripts print;
- **the flags** — `add_character_arguments()` and `add_look_arguments()` build
  the `-C`/`--random` and `-s`/`-m`/`-b`, `-c`/`-a`/`--no-color`, `-W` groups
  once, so the parsers can't drift apart. (`whonews` spends its `-n` on
  `--count`, so it passes `mono_flags=("--no-color",)`.)

The model backends are the one thing `whocast` does not hold: they live in
`whonews.py`, and `whoopinion.py` imports them from there — including
`add_model_arguments()`, which builds the provider, credential, endpoint
and model flags for both.
...
- `whoopinion.py` — your question, answered in a character's voice
